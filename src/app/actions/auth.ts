...
export const getAuthenticatedUser = cache(
  async (): Promise<ApiResponse<User>> => {
    // 1. Check NextAuth OAuth session first
    ...
    if (session?.user?.email) {
      ...
      if (dbUser) {
        return {
          ...
          data: {
            id: dbUser.id,
            username: dbUser.username || dbUser.name || "Chess Player",
            email: dbUser.email,
    ...
    if (!token.data)
      return {
        success: false,
        data: null,
        error: token.error || "JWT not found",
      };
    try {
  ...
  try {
    ...
    const user = await prisma.user.create({
      data: {
        ...
        rating: 600, // Default chess rating
      },
    });
...
export async function enforceUsername() {
  const authRes = await getAuthenticatedUser();
  if (authRes.success && authRes.data) {
    if (!authRes.data.username) {
